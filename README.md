...
The p-value is the proportion of bootstrap GoF statistics that are greater than or equal to the empirical Goodness of Fit (GoF) statistic, such that $p = (1/B) * sum_{i=1}^{B} I(GoF_i >= GoF_{empirical})\$, where $B$ is the number of bootstrap samples, $\mathbb{I}(.)$ is the indicator function, which is 1 if the condition is true and 0 otherwise, $GoF$ is the GoF statistic for the $i$-th bootstrap sample, and $GoF_{empirical}$ is the GoF statistic for the empirical data.
